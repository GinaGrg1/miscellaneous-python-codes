...
def validate(record):  # record --> Record(date=datetime.date(2000, 1, 1), snow=0, tmax=100, tmin=11, pgtm=datetime.time(13, 37))
    for attr, validator in validators:
        value = getattr(record, attr, _missing)  # getattr(record, 'date', object())
