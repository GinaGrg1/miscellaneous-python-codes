...
def load_countries(fp):
    """
    record = {'geoname_id':49518,'locale_code':'en','continent_code':'AF','continent_name':'Africa',
              'country_iso_code':'RW','country_name':'Rwanda','is_in_european_union':0}
    """
    countries = {}
    for record in csv.DictReader(fp):
        name, iso = record['country_name'], record['country_iso_code']
        countries[record['geoname_id']] = Country(name, iso)
    return countries     # {49518: Country(name='Rwanda', iso='RW')}
